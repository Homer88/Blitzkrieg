...
void CFormation::ProcessLoadCommand( CAICommand *pCommand, bool bPlaceInQueue )
{
	...
	if ( pCommand->GetFlag() == -1 )
	{
		...
		std::set<int> objects;
		stdext::hash_map<int, CAICommand*> object2Command;
		stdext::hash_set<int> formations;
...
//				if ( pFormation->IsFree() )
		...
		stdext::hash_map<int, int> availableSeats;
		int cnt = 0;
		for ( std::set<int>::iterator iterObjects = objects.begin(); iterObjects != objects.end(); ++iterObjects )
		{
			...
			for ( stdext::hash_set<int>::iterator iterForms = formations.begin(); iterForms != formations.end(); ++iterForms )
			{
