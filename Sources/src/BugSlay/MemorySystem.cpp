...
{
	// Required by stdext::hash_map in MSVC 2022
	enum { bucket_size = 4 };
	size_t operator()( const SFileLocationKey &key ) const
	{
		std::hash<std::string> HashFunc;
		return HashFunc( key.first ) + key.second;
	}
	// Also required for ordering within buckets
...
// Dummy equal - not used when hash func also provides comparison
